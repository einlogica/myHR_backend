        ...
        public function getActivity($data){
         ...
         public function post_Activity($data){
            ...
            if($stm->execute()===TRUE){
                ...
                $query = "SELECT * FROM `Attendance` WHERE Mobile = '$usermobile' AND Date = '$date' AND DATE = '$CurrDate'";
                ...
                if($rowCount===0){
                    ...
                    $query = "INSERT INTO `Attendance` (`Mobile`, `Name`, `PosLat`, `PosLong`,`Date`,`InTime`, `PosLat2`, `PosLong2`,`OutTime`,`Status`,`Location`,`Flag`) VALUES ('$usermobile','$username','$lat','$long','$CurrDate','$Time','0.00','0.00','00:00:00','Present','$place','false')";
                    $stm = $this->conn->prepare($query);
                    $stm->execute();
                    // if($stm->execute()){
