...
class call_api{
    ...
    public function updateLeave(){
        ...
        if($status==="Rejected"){
            ...
            if($per == "Manager"){
                ...
                UPDATE attendance RIGHT JOIN leavetracker ON attendance.`Mobile`=leavetracker.`Mobile` AND attendance.`Date`=leavetracker.`LeaveDate` SET attendance.`Status`='Absent',attendance.`Location`='Absent' WHERE leavetracker.`ID` in ($idValues);
                DELETE attendance FROM attendance INNER JOIN leavetracker ON attendance.`Mobile`=leavetracker.`Mobile` AND attendance.`Date`=leavetracker.`LeaveDate` WHERE attendance.`Date`='$CurrDate' AND leavetracker.`ID` in ($idValues)";
            }
            else if($per == "Admin"){
    ...
    public function addAdvance(){
        ...
        if($stm->execute()===TRUE){
            ...
            $status = $this->process_uploadBill($emp,$usermobile,$username,"Salary Advance","",0,"","$account","","",0,"",$amount,$CurrDate,"","false");
