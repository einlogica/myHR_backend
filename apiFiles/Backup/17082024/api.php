...
class call_api{
    ...
    public function upload_bill(){
        ...
        return $this->process_uploadBill($emp,$usermobile,$username,$type,$item,0,$site,$fromLoc,$toLoc,$km,$billno,$billamount,$billdate,$file,$fileavailable);
    ...
    private function process_uploadBill($emp,$usermobile,$username,$type,$item,$shop,$site,$fromLoc,$toLoc,$km,$billno,$billamount,$billdate,$file,$fileavailable){
        ...
        if($fileavailable=="true"){
            ...
            // move_uploaded_file($file["tmp_name"], $target_file
              ...
              if (file_put_contents($target_file, $data)) {
                $query = "INSERT INTO `ExpenseTracker` (`Mobile`,`Name`, `Site`,`FromLoc`,`ToLoc`,`KM`,`Shop`,`Item`, `Date`, `Type`, `BillNo`, `Amount`, `Filename`,`Status`) VALUES ('$usermobile','$username','$site','$fromLoc','$toLoc','$km','$shop','$item','$billdate','$type','$billno','$billamount','$filename','$status')";
                $stm = $this->conn->prepare($query);
                if($stm->execute()===TRUE){
                ...
                if($type==='Labour'){
                    ...
                    $query = "INSERT INTO `ExpenseTracker` (`Mobile`,`Name`, `Site`,`FromLoc`,`ToLoc`,`KM`,`Shop`,`Item`, `Date`, `Type`, `BillNo`, `Amount`, `Filename`,`Status`) VALUES ('$usermobile','$username','$site','$fromLoc','$toLoc','$km','$shop','$item','$billdate','$type','$billno','$billamount','NONE','$status')";
    ...
    public function get_preexpense(){
        ...
        $query = "SELECT Sum(Amount) AS Total FROM ExpenseTracker WHERE `Type` not LIKE '%Advance%' AND `Status`!='Rejected' AND Mobile = '$usermobile' AND Mobile IN (SELECT Mobile FROM `EmployeeInfo` WHERE `Employer`='$emp') AND MONTH(Date) = '$mon' AND YEAR(Date)='$year'";
        ...
        $query = "SELECT Sum(Amount) AS Advance FROM ExpenseTracker WHERE `Type`='Advance' AND Mobile = '$usermobile' AND Mobile IN (SELECT Mobile FROM `EmployeeInfo` WHERE `Employer`='$emp') AND MONTH(Date) = '$mon' AND YEAR(Date)='$year'";
    ...
    public function upload_purchasebill(){
        ...
        if($shopid!=""){
            // echo $shopid;
            $status = $this->process_uploadBill($emp,$usermobile,$username,$type,$item,$shopid,$site,"","",0,$billno,$billamount,$billdate,$file,$fileavailable);
    ...
    public function getCollection(){
        ...
        if($per==="User"){
            ...
            $query = "SELECT * FROM `collectiontracker` LEFT JOIN `billertracker` ON `collectiontracker`.`ShopID`=`billertracker`.`ShopID` WHERE `Date`='$date' AND `Mobile` in (SELECT `Mobile` FROM `EmployeeInfo` WHERE Employer='$emp' AND ManagerID in (SELECT EmployeeID FROM `EmployeeInfo` WHERE `Mobile`='$usermobile')";
        }
        else if($per==="Admin"){
    ...
    public function updateLeave(){
        ...
        if($status==="Rejected"){
            ...
            $query = "UPDATE `LeaveTracker` SET `L1Status`='$status', `L1Comments`='$comments', `Status`='L1 Approved' WHERE `ID` IN ($idValues)";
                ...
                if($chkCount>0){
                    $query = "INSERT INTO `Attendance` (`Mobile`,`Name`,`PosLat`,`PosLong`,`Date`,`InTime`,`PosLat2`,`PosLong2`,`OutTime`,`Status`,`Location`) SELECT `Mobile`,`Name`,'0.00','0.00','$CurrDate','00:00','0.00','0.00','00:00','Leave','Leave' FROM `LeaveTracker` WHERE `ID` in ($idValues) and `LeaveDate`='$CurrDate'";
                    $stm = $this->conn->prepare($query);
                    $stm->execute();
                }
    ...
    public function addAdvance(){
        ...
        if($stm->execute()===TRUE){
            ...
            $status = $this->process_uploadBill($emp,$usermobile,$username,"Salary Advance","",0,"","","",0,"",$amount,$CurrDate,"","false");
    ...
    public function getEvents(){
        ...
        $query = "SELECT `ID`,'Anniversary' AS `Event`,`Name` FROM `EmployeeInfo` WHERE MONTH(`DOJ`)=MONTH('$CurrDate') AND DAY(`DOJ`)=DAY('$CurrDate') AND `DOJ`!='$CurrDate' AND `Employer`='$emp'";
        $stm = $this->conn->prepare($query);
        if($stm->execute()===TRUE){
     ...
     public function get_tracker(){
        ...
        if($rowCount!=0){
            ...
            if($item==='Attendance'){
                ...
                $query = "SELECT * FROM `collectiontracker` LEFT JOIN `billertracker` ON `collectiontracker`.`ShopID`=`billertracker`.`ShopID` WHERE `Item`='Cash' AND `Mobile` in (SELECT `Mobile` FROM `EmployeeInfo` WHERE `Mobile` IN ($userValues) AND `Employer`='$emp') AND `collectiontracker`.`Date` BETWEEN '$fromDate' AND '$toDate' ORDER BY `Date`";
