...
class call_api{
    ...
    public function register(){
        ...
        if(!isset($_POST['usermobile']) || !isset($_POST['username']) || !isset($_POST['useremail'])){
            return;
        }
        ...
        $query = "INSERT INTO `EmployerInfo` (`EmpName`,`EmpShortname`,`AddressL1`,`AddressL2`) VALUES ('$employer','$short','$l1','$l2');
                    ...
                    INSERT INTO `PersonalData` (`Name`,`Mobile`) VALUES ('$username','$usermobile');
        ...
        $stm = $this->conn->prepare($query);
        if($stm->execute()===TRUE){
            $status = $this->sendEmail($username,$useremail,$pin,"new");
    ...
    public function get_userdetails(){
        ...
        if($filter==='ALL'){
            ...
            // $query = "SELECT Mobile,Name,EmployeeID,Department,Position,Manager,ManagerID,Permission,LeaveCount,LeaveBalance,ImageFile FROM `ExpenseUsers` WHERE `Mobile` ='$mobile' ";
            $query = "SELECT DISTINCT `UserInfo`.`Employer`,`EmployeeInfo`.*,`UserInfo`.`Permission`,`UserInfo`.`Email` FROM `EmployeeInfo` LEFT JOIN `UserInfo` ON `EmployeeInfo`.`Mobile`=`UserInfo`.`Mobile` WHERE `UserInfo`.`Permission`='Admin' AND `UserInfo`.`ID` in (SELECT MIN(`ID`) FROM `UserInfo` WHERE `Permission`='Admin' AND `Status`='ACTIVE' GROUP BY `Employer`)";
        }
        ...
        if($rowCount>0){
            ...
            if((int)$row['Users']>(int)$row['Count']){
                ...
                if($rowCount==0){
                    ...
                    $stm = $this->conn->prepare($query);
                    if($stm->execute()===TRUE){
                        $status = $this->sendEmail($user['Name'],$user['Email'],$pin,"newuser");
    ...
    public function resetPassword(){
        ...
        if($rowCount!=0){
            ...
            $status = $this->sendEmail($username,$useremail,$pin,"reset");
            ...
            if($status ==='Success'){
                ...
                if($stm->execute()===TRUE){
                    return "OTP shared to registered mailid";
                }
                else{
                    return "Failed";
                }
    ...
    private function sendEmail($name,$email,$otp,$type){
        ...
        try {
            ...
            $mail->Password = 'bP=1ae2+^s';  // SMTP password
    ...
    //================================================================================ Request for upgrade plan

    public function getQuote(){

        if (!isset($_POST["usermobile"]) || !isset($_POST["empCount"])) {
            return;
        }

        $usermobile=trim($_POST['usermobile']);
        $emp=trim($_POST['emp']);
        $empCount=trim($_POST['empCount']);
    ...
    //     $url = "https://cpaas.messagecentral.com/auth/v1/authentication/token?customerId=C-0B69574FAB4F4AC&key=TWVzc2FnZUAxOTg3&scope=NEW&country=91&email=jinuchandran@einlogica.com";
    ...
    //     // $authToken="eyJhbGciOiJIUzUxMiJ9.thisisatesttoken.IgGu7Sb4lovBSql5LIXZU3jlwPG4giAMZ2kTI Mg_EAaKVsVcCdpW_TYzyzdTd94GdEJMt5WntEGcEX4w0hITng";

    //     // echo $authToken;

    //     // Adding the authToken header
    //     $headers = [
    //         "Authorization: Bearer $authToken",
    //         "Content-Type: application/json", // Optional: Specify content type
    //     ];

    //     // $message = "Your%20password%20to%20login%20to%20myHR%20is%20".$otp; 
    //     $message = $otp." is your one time password (OTP) for user authentication from myHR Application - Powered by U2opia";

    //     $url = "https://cpaas.messagecentral.com/verification/v3/send?countryCode=91&flowType=SMS&mobileNumber=".$mobile."&senderId=C-0B69574FAB4F4AC&type=SMS&message=".$message."%3&messageType=OTP";
    //     echo $url;
    //     $ch = curl_init(); // Initialize cURL
    //     curl_setopt($ch, CURLOPT_URL, $url); // Set the URL
    //     curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Return response as a string
    ...
    //         "authorization: W0YvIFyUCbnkwDxserghp438qQdPjXNA5HEzuoZOVSJLf7GmK9uR7NmYlqaIwVBsWQKLxodkSz4MHgP8",
    //         "accept: */*",
